...
public class ChangeTableResource extends ServerResource {
	...
	public Object config(){
		...
		if(getReference().getPath().contains(SAVIRestRoute.CONVERT_TO_NONE)) {
			if (dpid.equals(DatapathId.NONE)) {
				saviProviderService.convertToNone();
			}else {
				saviProviderService.convertToNone(dpid);
			}
			return Collections.singletonMap("changeToNone", dpid + "success to change to none");
		}
