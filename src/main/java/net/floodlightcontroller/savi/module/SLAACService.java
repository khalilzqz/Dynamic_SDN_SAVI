...
public class SLAACService extends SAVIBaseService {
	...
	//	System.out.println("错误。。。。" + (ipv6.getPayload() instanceof IPacket));
	//	IPacket ipacket = ipv6.getPayload();
		//ipacket.
	...
	protected RoutingAction processNeighborSoliciation(SwitchPort switchPort,Ethernet eth){
		...
		//对不工整mac、ip进行过滤
		if(!macAddress.toString().contains("00:00:00:00") || !targetAddress.toString().contains("2001:")){
	...
	//		System.out.println("MAC和交换机端口绑定");
		}
	...
	//		System.out.println("manager绑定表已经存在"+eth.getSourceMACAddress()+"  "+targetAddress+ "    " + switchPort);
			return RoutingAction.FORWARD_OR_FLOOD;
		}
		else if(pool.isContain(targetAddress)){
			//log.info("pool is contain");
	//		System.out.println("manager绑定不存在，有ip地址"+eth.getSourceMACAddress()+"  "+targetAddress + "    " + switchPort);
			actions.clear();
			binding = pool.getBinding(targetAddress);
			/*if(binding.check(macAddress,targetAddress)){
			...
			actions.clear();
			binding = new Binding<>();
	//		System.out.println("绑定不存在，也没有ip地址"+eth.getSourceMACAddress()+"  "+targetAddress+ "    " + switchPort);
